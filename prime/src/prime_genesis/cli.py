...
def build_parser() -> argparse.ArgumentParser:
    ...
    status_parser.add_argument("--json", action="store_true", help="Compatibility flag; status output is JSON")
...
def _run_operator_command(args: argparse.Namespace) -> int | None:
    ...
    if args.command == "ready":
        report = evaluate_readiness(state_path=args.state, runtime_db_path=args.runtime_db)
        _print(report.to_dict())
        return 0 if report.ready else 1
    if args.command == "token-issue":
        print(issue_token(os.getenv("PRIME_AUTH_SECRET", ""), _context(args), ttl_seconds=args.ttl))
        return 0
...
def _run_runtime_command(args: argparse.Namespace) -> int:
    ...
    try:
        if args.command == "products":
            ...
            _print(asdict(runtime.ingest_memory(
                _context(args), namespace=args.namespace, content=args.content
            )))
        elif args.command == "memory-search":
            _print([asdict(item) for item in runtime.search_memory(
                _context(args), query=args.query, namespace=args.namespace, limit=args.limit
            )])
        elif args.command == "document-add":
            _print(asdict(runtime.ingest_document(
                _context(args),
